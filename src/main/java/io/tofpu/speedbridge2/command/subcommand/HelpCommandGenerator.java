...
public final class HelpCommandGenerator {
  ...
  private static final String HEADER =
      FIRST_CHARACTER + "<<gold>-</gold>> <yellow><bold>SpeedBridge <white>V2</bold>";
  private static final String COMMAND_FORMAT =
      String.format(SUBTITLE, "/%s " + "<dark_gray>- <white>%s");
  ...
  public static void generateHelpCommand(final @NotNull Plugin plugin) {
    ...
    messages.add(String.format(PAIR_FORMAT, "Version", plugin.getDescription().getVersion()));
    messages.add("");
    ...
    for (final Method method : declaredMethods) {
      ...
      messages.add(
          String.format(
              COMMAND_FORMAT,
              commandMethod.value().replace("speedbridge|sb", "sb"),
    ...
    messages.add(
        String.format(
            PAIR_FORMAT, "Discord", "<click:OPEN_URL:https://tofpu.me/discord>tofpu.me/discord"));
