...
public final class Leaderboard {
  ...
  public CompletableFuture<Void> load(final JavaPlugin javaPlugin) {
    ...
    BridgeUtil.runBukkitAsync(
        () -> {
          ...
          try (final DatabaseQuery databaseQuery =
              DatabaseQuery.query("SELECT DISTINCT *" + " FROM scores ORDER BY score")) {
                ...
                resultSet -> {
                  while (resultSet.next()) {
                    ...
                    if (uuidList.contains(uuid)) {
                      BridgeUtil.debug(
                          "Leaderboard#load(): uuidList contains " + uuid + "; continuing");
                      continue;
                    }
    ...
    BridgeUtil.runBukkitAsync(
        () -> {
          ...
          for (final UUID uuid : playerCache.asMap().keySet()) {
        ...
        20L * ConfigurationManager.INSTANCE.getLeaderboardCategory().getGlobalUpdateInterval(),
        20L * ConfigurationManager.INSTANCE.getLeaderboardCategory().getGlobalUpdateInterval());
    ...
    BridgeUtil.runBukkitAsync(
        () -> {
          ...
          for (final BridgePlayer bridgePlayer : PlayerService.INSTANCE.getBridgePlayers()) {
            ...
            if (bestScore != null) {
              ...
              scoreMap.put(
                  uuid, new BoardPlayer(bridgePlayer.getName(), position, uuid, bestScore));
            }
          }
        ...
        ConfigurationManager.INSTANCE.getLeaderboardCategory().getSessionUpdateInterval(),
  ...
  public CompletableFuture<BoardPlayer> retrieve(final UUID uniqueId) {
    final BoardPlayer player = playerCache.asMap().get(uniqueId);
  ...
  public BoardPlayer retrieve(
      final LeaderboardRetrieveType leaderboardRetrieveType, final int position) {
  ...
  public CompletableFuture<IslandBoardPlayer.IslandBoard> retrieve(
      final UUID uniqueId, final int islandSlot) {
    final IslandBoardPlayer player = islandPositionMap.asMap().get(uniqueId);
    ...
    return PluginExecutor.supply(
        () -> islandPositionMap.getUnchecked(uniqueId).retrieve(islandSlot));
  }
