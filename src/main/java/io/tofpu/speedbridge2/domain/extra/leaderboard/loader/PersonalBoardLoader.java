...
public final class PersonalBoardLoader extends CacheLoader<UUID, BoardPlayer>
    implements BoardRetrieve<BoardPlayer> {
  public static final PersonalBoardLoader INSTANCE = new PersonalBoardLoader();
  private static final String GLOBAL_POSITION =
      "SELECT DISTINCT 1 + COUNT(*) AS "
  ...
  @Override
  public ListenableFuture<BoardPlayer> reload(
      final @NotNull UUID key, final @NotNull BoardPlayer oldValue) {
    ...
    try (final DatabaseQuery databaseQuery = DatabaseQuery.query(GLOBAL_POSITION)) {
      ...
      if (player == null) {
        return new BoardPlayer(
            PlayerNameCache.INSTANCE.getOrDefault(key), 0, key, new Score(-1, -1));
      }
      return player;
  ...
  public BoardPlayer toBoardPlayer(final UUID uid, final DatabaseSet databaseSet) {
    return new BoardPlayer(
        PlayerNameCache.INSTANCE.getOrDefault(uid),
        databaseSet.getInt("position"),
        uid,
        new Score(-1, -1));
  }
}
