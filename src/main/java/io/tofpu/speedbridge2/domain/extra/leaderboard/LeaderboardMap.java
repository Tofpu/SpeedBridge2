...
public final class LeaderboardMap extends LinkedHashMap<Integer, BoardPlayer> {
  ...
  public void updateLeaderboard() {
    ...
    final LinkedHashMap<Integer, BoardPlayer> sortedMap =
        tempClone.entrySet().stream()
            .filter(uuidScoreEntry -> !removeUserList.contains(uuidScoreEntry.getKey()))
            .sorted(Map.Entry.comparingByValue())
            .limit(10)
            .collect(
                Collectors.toMap(
                    o -> positionCounter.incrementAndGet(),
                    o -> {
                      final UUID uuid = o.getKey();
                      return new BoardPlayer(
                          PlayerNameCache.INSTANCE.getOrDefault(uuid),
                          positionCounter.get(),
                          uuid,
                          o.getValue());
                    },
    ...
    final Map<Integer, BoardPlayer> newBoardMap =
        globalBoardCopy.entrySet().stream()
            .filter(
                uuidScoreEntry -> !removeUserList.contains(uuidScoreEntry.getValue().getOwner()))
