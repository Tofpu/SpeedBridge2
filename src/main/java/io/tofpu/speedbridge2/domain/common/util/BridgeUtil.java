...
public final class BridgeUtil {
  ...
  public static Component sendMessage(final CommandSender sender, final Component component) {
    final Audience audience = SpeedBridge.getAdventure().sender(sender);
    audience.sendMessage(component);
    return component;
  }

  public static Component sendMessage(final CommonBridgePlayer<?> sender, final String content) {
    final Component component = translateMiniMessage(content);
    sendMessage(sender.getPlayer(), component);
    return component;
  }

  public static Component sendMessage(final CommandSender sender, final String content) {
    final Component component = translateMiniMessage(content);
  ...
  public static UUID findUUIDBy(final String playerName) {
    ...
    try (final DatabaseQuery databaseQuery =
        DatabaseQuery.query("SELECT uid FROM " + "players WHERE name " + "= ?")) {
  ...
  public static void runBukkitAsync(final Runnable runnable) {
    Bukkit.getScheduler()
        .runTaskAsynchronously(JavaPlugin.getPlugin(SpeedBridgePlugin.class), runnable);
  }

  public static void runBukkitAsync(
      final Runnable runnable, final long delay, final long interval) {
    Bukkit.getScheduler()
        .runTaskTimerAsynchronously(
            JavaPlugin.getPlugin(SpeedBridgePlugin.class), runnable, delay, interval);
  }

  public static void whenComplete(
      final CompletableFuture<?> completableFuture, final Runnable whenComplete) {
