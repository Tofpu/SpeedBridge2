...
public final class Message {
  ...
  private static final String SCORE_TITLE_BAR =
      MessageUtil.CHAT_BAR.substring(0, MessageUtil.CHAT_BAR.length() / 6);
  ...
  private static final String SUCCESS =
      "<gold><bold>" + MessageUtil.Symbols.ARROW_RIGHT.getSymbol() + "</bold> <yellow>";
  ...
  public final String islandSetupNotification =
      SUCCESS
          + "You can complete your %slot% island setup by running "
          + runCommand("/sb setup %slot%")
          + ", and setting up the spawn point.";
  ...
  public final String invalidIslandArgument =
      error
          + "Invalid argument. Please choose a slot, or an island category"
          + ".\n"
          + error
          + "Alternatively, you could run the '"
          + runCommand("/randomjoin")
          + "' command.";
  public final String invalidIsland = error + "Island %s cannot be found!";
  public final String noAvailableIsland =
      error + "There is no island available " + "at the moment... please try again " + "later!";
  ...
  public final String scoreTitle =
      "<yellow>"
          + SCORE_TITLE_BAR
          + "<gold><bold"
  ...
  public final String lobbyMissing =
      error
          + "Incomplete setup. Please ensure to set up SpeedBridge's lobby to "
          + "complete the "
          + "process."
          + "\n<red>Type "
          + runCommand("/speedbridge setlobby")
          + " to set the "
          + "lobby.";
  ...
  public final String completeNotification =
      SUCCESS + "You can complete the setup " + "by typing " + runCommand("/sb setup finish");

  public final String setupIncomplete =
      error + "The setup is incomplete. Please " + "ensure that the spawn point is set.";
  ...
  private static String runCommand(final String command) {
    return "<hover:show_text:'<yellow>Click to run "
        + "%command%'><click:run_command:'%command%'>%command%".replace("%command" + "%", command);
  }
    ...
    return CompletableFuture.runAsync(
        () -> {
          ...
          for (final String fileMessage : FileUtil.read(messageFile)) {
            ...
            final String message = substring(args, 1).replaceFirst(" ", "").replace("\\n", "\n");
