...
public final class IslandSetup {
  ...
  public IslandSetup(
      final BridgePlayer playerEditor, final Island island, final IslandPlot islandPlot) {
  ...
  public boolean finish() {
    ...
    final Location absoluteLocation = islandPlot.getPlotLocation().subtract(this.playerSpawnPoint);
    ...
    playerEditor
        .getPlayer()
        .teleport(ConfigurationManager.INSTANCE.getLobbyCategory().getLobbyLocation());
  ...
  private void resetPlot() {
    final ClipboardWrapper clipboardWrapper =
        MultiWorldEditAPI.getMultiWorldEdit().create(island.getSchematicClipboard());
    ...
    for (int x = 0; x < maximumPoint.getX() - minimumPoint.getX() + offset; x++) {
      for (int y = 0; y < maximumPoint.getY() - minimumPoint.getY() + offset; y++) {
        for (int z = 0; z < maximumPoint.getZ() - minimumPoint.getZ() + offset; z++) {
          ...
          this.islandPlot.getWorld().getBlockAt(blockX, blockY, blockZ).setType(Material.AIR);
        }
      }
    }
  }
