...
public final class IslandDatabase extends Database {
  ...
  public @NotNull CompletableFuture<Void> insert(final Island island) {
    return DatabaseUtil.databaseQueryExecute(
        "INSERT OR IGNORE INTO islands VALUES " + "(?, ?, ?, ?)",
        databaseQuery -> {
  ...
  public @NotNull CompletableFuture<Void> update(final Island island) {
    return DatabaseUtil.databaseQueryExecute(
        "UPDATE islands SET category = ?, schematic_name = " + "?, spawn_point = ? WHERE slot = ?",
        databaseQuery -> {
  ...
  public @NotNull CompletableFuture<Void> delete(final int slot) {
    return DatabaseUtil.databaseQueryExecute(
        "DELETE FROM islands WHERE slot = ?",
        databaseQuery -> {
  ...
  public @NotNull CompletableFuture<List<Island>> getStoredIslands() {
    return runAsync(
        () -> {
          ...
          try {
            DatabaseUtil.databaseQuery(
                    "SELECT * FROM islands",
                    resultSet -> {
                      while (resultSet.next()) {
                        final Island island =
                            new Island(resultSet.getInt("slot"), resultSet.getString("category"));
                        ...
                        if (spawnPoint != null) {
                          ...
                          island.setAbsoluteLocation(
                              new Location(Bukkit.getWorld("Speedbridge2"), x, y, z, yaw, pitch));
                        }
