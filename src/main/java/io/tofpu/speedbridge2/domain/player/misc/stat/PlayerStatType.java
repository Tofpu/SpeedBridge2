...
public enum PlayerStatType {
  ...
  public static PlayerStat create(
      final UUID owner, final PlayerStatType playerStatType, final String value) {
    switch (playerStatType.name()) {
      case "TOTAL_WINS":
      case "TOTAL_TRIES":
        return new SimplePlayerStat(
            playerStatType.name().toLowerCase(Locale.ENGLISH), owner, Integer.parseInt(value));
    }
    return null;
  }
