...
public final class PlayerDatabase extends Database {
  ...
  public @NotNull CompletableFuture<Void> update(final @NotNull BridgePlayer player) {
    ...
    completableFutures.add(updateName(player.getPlayer().getName(), player));
    ...
    return runAsync(
        () -> {
          try (final DatabaseQuery databaseQuery =
              DatabaseQuery.query("UPDATE players SET name = ? WHERE uid" + " = ?")) {
  ...
  public @NotNull CompletableFuture<BridgePlayer> getStoredPlayer(final @NotNull UUID uniqueId) {
    return runAsync(
        () -> {
          ...
          try {
            final Collection<Score> scoreList =
                Databases.SCORE_DATABASE.getStoredScore(uniqueId).get();

            final Collection<PlayerStat> playerStats =
                Databases.STATS_DATABASE.getStoredStats(bridgePlayer.getPlayerUid()).get();

            Material material = Databases.BLOCK_DATABASE.getStoredMaterial(uniqueId).get();
