...
public final class ScoreDatabase extends Database {
  ...
  public @NotNull CompletableFuture<Void> insert(
      final @NotNull UUID uuid, final @NotNull Score score) {
  ...
  public @NotNull CompletableFuture<Void> update(
      final @NotNull UUID uuid, final @NotNull Score score) {
    return DatabaseUtil.databaseQueryExecute(
        "UPDATE scores SET island_slot = ?, " + "score = ? WHERE uid = ?",
        databaseQuery -> {
          BridgeUtil.debug("player uid: " + uuid);
    ...
    return PluginExecutor.supply(
        () -> {
          ...
          try (final DatabaseQuery query =
              DatabaseQuery.query("SELECT * FROM scores WHERE uid = ?")) {
            ...
            query.executeQuery(
                resultSet -> {
                  while (resultSet.next()) {
                    final Score score =
                        Score.of(resultSet.getInt("island_slot"), resultSet.getDouble("score"));
  ...
  public CompletableFuture<?> delete(final UUID uuid) {
    return PluginExecutor.runAsync(
        () -> {
          try (final DatabaseQuery query =
              DatabaseQuery.query("DELETE FROM scores " + "WHERE uid = ?")) {
