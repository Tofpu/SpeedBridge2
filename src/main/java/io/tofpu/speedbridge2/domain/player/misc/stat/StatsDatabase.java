...
public final class StatsDatabase extends Database {
  ...
  public @NotNull CompletableFuture<Void> insert(final @NotNull PlayerStat playerStat) {
    return DatabaseUtil.databaseQueryExecute(
        ...
        databaseQuery -> {
          databaseQuery.setString(playerStat.getOwner().toString());
  ...
  public @NotNull CompletableFuture<Void> update(final @NotNull PlayerStat playerStat) {
    return DatabaseUtil.databaseQueryExecute(
        "UPDATE stats SET value = ? WHERE " + "uid = ? AND key = ?",
        databaseQuery -> {
          databaseQuery.setString(playerStat.getValue());
  ...
  public @NotNull CompletableFuture<Collection<PlayerStat>> getStoredStats(
                          ...
                          resultSet -> {
                            while (resultSet.next()) {
                              final PlayerStatType playerStatType =
                                  PlayerStatType.match(resultSet.getString("key"));

                              if (playerStatType == null) {
                                continue;
                              }
                              final PlayerStat playerStat =
                                  PlayerStatType.create(
                                      owner, playerStatType, resultSet.getString("value"));
    ...
    return PluginExecutor.runAsync(
        () -> {
          try (final DatabaseQuery query =
              DatabaseQuery.query("SELECT * FROM stats WHERE " + "uid = ?")) {
