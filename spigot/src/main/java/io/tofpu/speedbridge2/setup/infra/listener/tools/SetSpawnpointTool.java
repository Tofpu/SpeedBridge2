...
public class SetSpawnpointTool extends SetupTool {
    public SetSpawnpointTool(SetupService setupService, String id, ItemStack item) {
    ...
    private static @NotNull ToolAction<PlayerInteractEvent> eventHandler(SetupService setupService) {
        return (genericToolbar, event) -> {
            setupService.islandSetup(event.getPlayer()).ifPresent(setup -> {
                setup.handleSetSpawnPoint();
                event.getPlayer().sendMessage("§aSpawnpoint set successfully!");
            });
        };
    }
}
